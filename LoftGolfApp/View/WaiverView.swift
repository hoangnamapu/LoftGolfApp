...
struct WaiverView: View {
    ...
    let onSigned: (_ signedName: String, _ signedAt: String) -> Void
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                ...
                7. Only one person should be present in the hitting area at a time.
                ...
                15. Customers acknowledge Loft Golf Studios is under video surveillance.
                ...
                17. Customers are responsible for reviewing the simulator system operation at https://loftgolfstudios.com/simulator-how-to
                18. All groups must leave the simulator bay 5 minutes prior to the incoming group start time.
                19. Failure to abide by any of the above procedures may result in cancellation of the remainder of paid simulator time without compensation.
                ...
                I HEREBY ACKNOWLEDGE THAT THIS DOCUMENT IS VALID AND MAY BE ENFORCED IN THE SAME MANNER AS A HAND-SIGNED DOCUMENT.

                I have carefully read and understood this Waiver and Release of Liability, and I voluntarily agree to its terms.
                """)
                ...
                Button {
                    handleWaiverSubmission()
                } label: {
                    Text("Agree and Continue")
                        .frame(maxWidth: .infinity)
                        .padding()
                        .background(agreed && !trimmedSignedName.isEmpty ? Color.white : Color.gray)
                        .foregroundColor(.black)
                        .cornerRadius(10)
                }
                .disabled(!agreed || trimmedSignedName.isEmpty)
    ...
    private var trimmedSignedName: String {
        signedName.trimmingCharacters(in: .whitespacesAndNewlines)
    }

    private func handleWaiverSubmission() {
        guard agreed && !trimmedSignedName.isEmpty else {
            showError = true
            return
        }
        ...
        onSigned(trimmedSignedName, signedAt)
        dismiss()
    }
}
