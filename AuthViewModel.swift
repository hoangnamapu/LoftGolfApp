...
extension AuthViewModel {
    func getCustomer() async throws -> CustomerProfile {
        ...
        for base in USAuthConfig.bases {
            do {
                var req = try makeRequest(base: base, path: "customer", method: "GET")
                req.setValue(token, forHTTPHeaderField: "X-US-AuthToken")

                let (data, resp) = try await URLSession.shared.data(for: req)
                guard let http = resp as? HTTPURLResponse else {
                    throw USAuthError.unknown(URLError(.badServerResponse))
                }
                ...
                let customer = try JSONDecoder().decode(USCustomerResponse.self, from: data)
...
private struct USCustomerResponse: Decodable {
    let Reference1: String?
    let Reference2: String?
    let Reference3: String?
    let StatusID: Int?
